...
def home(request):
    ...
    visit.home_visit_count += 1
    visit.save()
    return render(request,'landing/EN/home.html',data)

def about(request):
    visit, created = PageVisit.objects.get_or_create(id=1)
    visit.about_visit_count += 1
    visit.save()
...
def portfolio(request):
    visit, created = PageVisit.objects.get_or_create(id=1)
    visit.portfolio_visit_count += 1
    visit.save()

    default_slide = random.choice([1, 2, 3])
    if request.method == 'POST':
        form = NewsletterForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('landing-portfolio')  # Redirect after POST to avoid form resubmission
    else:
        form = NewsletterForm()
    
    return render(request, 'landing/EN/landing_page.html' , {'form': form,'visit_count': visit.portfolio_visit_count, 'default_slide': default_slide})
